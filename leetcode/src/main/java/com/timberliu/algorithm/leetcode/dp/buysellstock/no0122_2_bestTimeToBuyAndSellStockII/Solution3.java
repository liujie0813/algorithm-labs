 ...
 *
 *  推导：
 *    如果第 i 天持有股票，可以由两个状态推导出来
 *      - 第 i-1 天已经持有股票，保持现状，结果就是昨天持有股票所得的最多现金 dp[i] = dp[i - 1][0]
 *      - 第 i 天买入股票，结果就是昨天不持有股票所得的现金 减去今天的股票价格 dp[i] = dp[i - 1][1] - prices[i]
 *    	dp[i][0] = max(dp[i - 1][0], dp[i - 1][1] - prices[i])
 *
 *    与 121 不同的是，121 只能买卖一次，所以第 i 天买入股票，所得现金一定是 -prices[i]
 *                 而 122 可以买卖多次，所以第 i 天买入股票，所得现金可能包含之前买卖所得的利益
 *
 *    如果第 i 天不持有股票：
