package com.timberliu.algorithm.leetcode.binary_search.no0153_2_findMinimumInRotatedSortedArray;
 ...
 *   原理？
 *     情况：a. 1,2,3,4,5,6,7
 *             递增：left < mid < right，收缩左边界
 *          b. 6,7,1,2,3,4,5
 *             旋转1：left > mid, mid < right, 收缩右边界
 *          c. 3,4,5,6,7,1,2
 *             旋转2：left < mid, mid > right, 收缩左边界
 *          d. 7,6,5,4,3,2,1
 *             旋转3：left > mid, mid > right, 收缩右边界
 *
 *     1. left < mid, 收缩左边界，left = mid
 *     2. left > mid, 收缩右边界，right = mid - 1
 *     3. 需要检查具体值，left = 0, right = n - 1
 *     4. 退出条件 left < right，取例分析，如 1、2、3 个数
 *          d. 2,1
 *             left=0,right=1,mid=1  nums[left]>nums[mid]
